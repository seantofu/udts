...
- 目标库为 UCloud Redis 分布式 UDRedis 版本时，目标库类型选择分布式版，地址为代理地址。
- 目标库为 UCloud Redis 分布式 Cluster 版本时，目标库类型选择集群版，地址为分片地址，多个地址之间用";"相连。
- 目标库为 UCloud Redis 分布式 Cluster 版本支持多db迁移。
- 目标库为开源集群版时，只支持迁移db0。
